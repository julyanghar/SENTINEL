...
class Qwen2_5_VLModel:
    def __init__(
        ...
        """
        Initialize Qwen2.5-VL model, load different models according to size.

        Args:
            use_vllm (bool): Whether to use vLLM.
            debug (bool): Whether to enable debug mode.
            model_size (str): Model size, '3B' or '7B'.
        ...
        assert model_size in [
            "3B",
            "7B",
        ], "Invalid model size. Choose from '3B'or '7B', pay attention to the capitalization."
        if logger:
            logger.info(f"Loading Qwen2.5 VL model with size {model_size} on {device} util {gpu_util*100}%.")
    ...
    def _create(self) -> tuple[LLM | Qwen2_5_VLForConditionalGeneration, Qwen2_5_VLProcessor]:
        if self.use_vllm:
            return self._create_vllm()
        else:
            return self._create_hf()

    def _create_vllm(self) -> list[LLM, Qwen2_5_VLProcessor]:
        from ..utils.gen_utils import init_vllm

        model_name = f"Qwen/Qwen2.5-VL-{self.model_size}-Instruct"
        ...
        processor: Qwen2_5_VLProcessor = AutoProcessor.from_pretrained(
            model_name,
            cache_dir=None,
        )
    ...
    def _create_hf(self) -> tuple[Qwen2_5_VLForConditionalGeneration, Qwen2_5_VLProcessor]:
        model_name = f"Qwen/Qwen2.5-VL-{self.model_size}-Instruct"
